...
public class ProgrammingArticlesRoute implements Route, LoginSupport {
	...
	public Object handle(Request req, Response res) throws Exception {
		...
		if("metas".equalsIgnoreCase(type)) {
			res.type(JSON_MIME);
			return metas();
		}
		...
		try {
			json.keySet().forEach(s -> {
				if(s.equals(TAGS)) {
					...
					String tags = arry.isEmpty() ? "" : Tag.serialize(IntStream.range(0, arry.length()).map(n -> sm.getTagByName(arry.getString(n)).getId()));
	...
	private synchronized Object create(Request req, Response res, String type, String numS) throws NumberFormatException, Exception {
		res.type(JSON_MIME);
	...
	private Renderable tags() throws Exception {
		init();
		return (sb, model) -> {
